...
export const getAccessCode = async () => {
  const accessCode = await app
    ...
    .then((snapshot) => {
      ...
      const timestampInMs = today.getTime();
      const unixTimestamp = Math.floor(timestampInMs / 1000);

      if (unixTimestamp < current_expires_at) {
        // const getTokenToken = axios.post('https://www.printful.com/oauth/token', {
        ...
        // console.log({
        //   rightNow: Date.now(),
        //   tokenExpiration: current_expires_at,
        // });
...
// const access_code = 'gVsERnIXDCBrNRSADb7DR0GiV2atPSb4OlgOkkWm';
